...
$$nemoclaw: a device scope upgrade is waiting for approval inside sandbox 'my-assistant'.
  ...
  Open a prepared shell:   $$nemoclaw my-assistant connect
...
Enter the prepared `connect` shell, read `openclaw devices list --json`, confirm the requesting client, device, and requested scopes, approve that request by id in the same shell, then re-run the original command unchanged.
