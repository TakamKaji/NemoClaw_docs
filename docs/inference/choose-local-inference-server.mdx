...
description-agent: "Compares NemoClaw local inference options. Use when choosing Ollama, vLLM, llama.cpp, NVIDIA NIM, or a fixed local model profile."
...
| Fixed local model profile | You need one catalog-selected model and serving runtime combination. | Requires the dedicated installer flag on DGX Spark and does not appear in the provider menu. | `/v1/chat/completions`. |
...
## Install a Fixed Local Model Profile

Use this feature-gated path to install one catalog-selected model-and-runtime combination for vLLM or llama.cpp.
...
- Stop any server that already uses port `8000` for vLLM or port `8081` for llama.cpp.
...
<AgentOnly variant="deepagents">
  ...
  NEMOCLAW_SANDBOX_NAME=my-assistant \
  NEMOCLAW_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
  bash -s -- --local-model-runtime=llama-cpp
```

The flag enables the dedicated profile gate, disables Express selection, and selects the matching catalog recipe.
Do not combine this path with `NEMOCLAW_PROVIDER` or `NEMOCLAW_MODEL`.
The vLLM profile rejects `NEMOCLAW_VLLM_MODEL`, `NEMOCLAW_VLLM_PORT`, and `NEMOCLAW_VLLM_EXTRA_ARGS_JSON` before installation.

The vLLM choice performs these actions:
...
The llama.cpp choice performs these actions:

- Downloads one exact GGUF artifact over HTTPS.
- Verifies the declared size and SHA-256 digest before cache publication or launch.
- Stores the verified artifact in an owner-only content-addressed cache.
- Publishes the authenticated server only on `127.0.0.1:8081` through a dedicated internal Docker network.

After either runtime passes its readiness check, onboarding registers the provider and routes sandbox traffic through `inference.local`.
...
Verify the loopback publication for the selected runtime:
...
Run only the command for the selected runtime.
The vLLM result must contain `127.0.0.1:8000`.
The llama.cpp result must contain `127.0.0.1:8081`.
...
If Docker reports a container-name or network-name conflict, inspect the resource labels before removing anything.
Do not remove a resource that lacks the NemoClaw ownership label for this profile.
Refer to [Host Files and State](../../reference/host-files-and-state) before deleting a credential or cached artifact.

When you intend to remove the entire NemoClaw installation, run `$$nemoclaw uninstall`.
Full uninstall verifies the exact managed container and network ownership before it removes those resources and deletes their state.
It preserves the managed llama.cpp model cache by default and preserves the shared Hugging Face cache used by vLLM.
Add `--delete-models` only when you also accept deletion of the managed llama.cpp cache and NemoClaw-pulled Ollama models.
...
After a successful full uninstall, `docker container inspect` must report no object for the selected managed container name.
For llama.cpp, `docker network inspect nemoclaw-llama-cpp-internal` must also report no object.
