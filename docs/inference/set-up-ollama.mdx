...
On DGX Spark and other local-inference hosts, you can use one local server for chat and a separate Ollama container for OpenClaw memory-search embeddings.
For example, managed vLLM can serve the chat model while a host-side Ollama container serves `qwen3-embedding:4b` on port `11434`.

The sandbox reaches that container through `http://host.openshell.internal:11434`.
The `local-inference` policy preset allows that egress path, while `config set` separately validates that the bridge URL is written only to supported provider `baseUrl` fields.

First make sure the embedding server is reachable from the sandbox:
...
Then define an OpenClaw provider for the embedding model and point memory search at it:
...
If your memory index already contains documents, check whether OpenClaw reports an index-identity mismatch, rebuild the index, then verify the configured memory provider:
...
Add `--agent <id>` to those `openclaw memory` commands when you are configuring a specific non-default OpenClaw agent.

Keep the Ollama container bound only to the local host or a reviewed host interface.
Port `11434` is commonly unauthenticated, so allow it only from the OpenShell Docker bridge or the equivalent local container network path.
